...
use SwooleKit\CoroDatabase\CoMysqlExpression;

/**
 * 语句生成器
 * Class CoMysqlBuilder
...
class CoMysqlBuilder
{
     ...
     * 架构函数
     * @access public
     * @param CoMysqlConnection $connection 数据库连接对象实例
     */
    public function __construct(CoMysqlConnection $connection)
    {
     ...
     */
    protected function parseData(CoMysqlQuery $query, $data = null, $fields = null, $bind = null)
    {
        if (empty($data)) {
            return [];
        }
        ...
        foreach ($data as $key => $val) {
            ...
            if ($val instanceof CoMysqlExpression) {
            ...
            if (false !== strpos($key, '->')) {
                ...
                if ($options['strict']) {
                    throw new CoMysqlException('fields not exists:[' . $key . ']');
                }
            } elseif (is_null($val)) {
                $result[$item] = 'NULL';
                ...
                switch (strtoupper($val[0])) {
                    ...
                    case 'DEC':
                        ...
                        break;
                    case 'EXP':
                        throw new CoMysqlException('not support data:[' . $val[0] . ']');
                }
     ...
     * @return string
     */
    protected function parseDataBind(CoMysqlQuery $query, $key, $data, $bind = null)
    {
        if ($data instanceof CoMysqlExpression) {
     ...
     * @throws CoMysqlException
     */
    protected function parseField(CoMysqlQuery $query, $fields)
    {
     ...
     * @throws CoMysqlException
     */
    protected function parseTable(CoMysqlQuery $query, $tables)
    {
        $item = [];
        $options = $query->getOptions();
     ...
     * @throws CoMysqlException
     */
    protected function parseWhere(CoMysqlQuery $query, $where)
    {
        $options = $query->getOptions();
     ...
     * @throws CoMysqlException
     */
    public function buildWhere(CoMysqlQuery $query, $where)
    {
        if (empty($where)) {
            $where = [];
        }
        ...
        foreach ($where as $logic => $val) {
            ...
            foreach ($val as $value) {
                if ($value instanceof CoMysqlExpression) {
                ...
                if (is_array($value)) {
                    if (key($value) !== 0) {
                        throw new CoMysqlException('where express error:' . var_export($value, true));
                    }
     ...
     * @throws CoMysqlException
     */
    protected function parseWhereItem(CoMysqlQuery $query, $field, $val, $rule = null, $binds = null)
    {
        // 字段分析
        ...
        if (!isset($whereStr)) {
            throw new CoMysqlException('where express error:' . $exp);
        }
     ...
     * @return string
     */
    protected function parseLike(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType, $logic)
    {
        // 模糊匹配
        if (is_array($value)) {
            foreach ($value as $item) {
     ...
     * @param string $key
     * @param string $exp
     * @param CoMysqlExpression $value
     * @param string $field
     * @param integer $bindType
     * @return string
     */
    protected function parseExp(CoMysqlQuery $query, $key, $exp, \think\db\Expression $value, $field, $bindType)
    {
        // 表达式查询
     ...
     * @throws CoMysqlException
     */
    protected function parseColumn(CoMysqlQuery $query, $key, $exp, array $value, $field, $bindType)
    {
        // 字段比较查询
     ...
     * @return string
     */
    protected function parseNull(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
        // NULL 查询
        return $key . ' IS ' . $exp;
    }
     ...
     * @return string
     */
    protected function parseBetween(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
        // BETWEEN 查询
     ...
     * @throws CoMysqlException
     */
    protected function parseExists(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
        // EXISTS 查询
        if ($value instanceof \Closure) {
            $value = $this->parseClosure($query, $value, false);
        } elseif ($value instanceof CoMysqlExpression) {
            $value = $value->getValue();
        } else {
            throw new CoMysqlException('where express error:' . $value);
        }
     ...
     * @return string
     */
    protected function parseTime(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
     ...
     * @throws CoMysqlException
     */
    protected function parseCompare(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
        if (is_array($value)) {
            throw new CoMysqlException('where express error:' . $exp . var_export($value, true));
        }
     ...
     * @return string
     */
    protected function parseBetweenTime(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
        if (is_string($value)) {
            $value = explode(',', $value);
        }
     ...
     * @return string
     */
    protected function parseIn(CoMysqlQuery $query, $key, $exp, $value, $field, $bindType)
    {
        // IN 查询
        if ($value instanceof \Closure) {
            $value = $this->parseClosure($query, $value, false);
     ...
     * @return string
     */
    protected function parseClosure(CoMysqlQuery $query, $call, $show = null)
    {
        $newQuery = $query->newQuery()->setConnection($this->connection);
        $call($newQuery);
     ...
     * @return string
     */
    protected function parseDateTime(CoMysqlQuery $query, $value, $key, $bindType = null)
    {
        $options = $query->getOptions();
     ...
     * @return string
     */
    protected function parseLimit(CoMysqlQuery $query, $limit)
    {
     ...
     * @throws CoMysqlException
     */
    protected function parseJoin(CoMysqlQuery $query, $join)
    {
        $joinStr = '';
     ...
     * @throws CoMysqlException
     */
    protected function parseOrder(CoMysqlQuery $query, $order)
    {
        foreach ($order as $key => $val) {
            if ($val instanceof CoMysqlExpression) {
                ...
                if (preg_match('/^[\w\.]+$/', $key)) {
                    ...
                    throw new CoMysqlException('order express error:' . $key);
                }
            }
        }
     ...
     * @throws CoMysqlException
     */
    protected function parseGroup(CoMysqlQuery $query, $group)
    {
     ...
     * @return string
     */
    protected function parseHaving(CoMysqlQuery $query, $having)
    {
     ...
     * @return string
     */
    protected function parseComment(CoMysqlQuery $query, $comment)
    {
     ...
     * @return string
     */
    protected function parseDistinct(CoMysqlQuery $query, $distinct)
    {
     ...
     * @return string
     */
    protected function parseUnion(CoMysqlQuery $query, $union)
    {
        if (empty($union)) {
            return '';
        }
     ...
     * @return string
     */
    protected function parseForce(CoMysqlQuery $query, $index)
    {
        if (empty($index)) {
            return '';
        }
     ...
     * @return string
     */
    protected function parseLock(CoMysqlQuery $query, $lock = null)
    {
        if (is_bool($lock)) {
     ...
     * @return string
     */
    protected function parseRegexp(CoMysqlQuery $query, $key, $exp, CoMysqlExpression $value, $field)
    {
     ...
     * @throws CoMysqlException
     */
    public function parseKey(CoMysqlQuery $query, $key, $strict = null)
    {
        if (is_numeric($key)) {
            return $key;
        } elseif ($key instanceof CoMysqlExpression) {
            return $key->getValue();
        }
        ...
        if (strpos($key, '->') && false === strpos($key, '(')) {
            ...
            // JSON字段支持 where('info->nickname','ThinkPHP')
        ...
        if ($strict && !preg_match('/^[\w\.\*]+$/', $key)) {
            throw new CoMysqlException('not support data:' . $key);
        }

        // 字段名处理 对于不是*的关键词均需要加上反引号保证语法
     ...
     * @return string
     */
    protected function parseRand(CoMysqlQuery $query)
    {
        return 'rand()';
    }
     ...
     * @throws CoMysqlException
     */
    public function select(CoMysqlQuery $query)
    {
        $options = $query->getOptions();
     ...
     * @return string
     */
    public function insert(CoMysqlQuery $query, $replace = null)
    {
        $options = $query->getOptions();
     ...
     * @throws CoMysqlException
     */
    public function insertAll(CoMysqlQuery $query, $dataSet, $replace = null)
    {
        $options = $query->getOptions();
     ...
     * @throws CoMysqlException
     */
    public function selectInsert(CoMysqlQuery $query, $fields, $table)
    {
        if (is_string($fields)) {
        ...
        return /** @lang text */ 'INSERT INTO ' . $this->parseTable($query, $table) . ' (' . implode(',', $fields) . ') ' . $this->select($query);
    }
     ...
     * @throws CoMysqlException
     */
    public function update(CoMysqlQuery $query)
    {
        $options = $query->getOptions();
     ...
     * @throws CoMysqlException
     */
    public function delete(CoMysqlQuery $query)
    {
        $options = $query->getOptions();
